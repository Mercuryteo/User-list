...
const userLists = JSON.parse(localStorage.getItem('favouriteUsers'))    //從localstorage取得用戶資料，去JSON化，並宣告該資料
  ...
  data.forEach((user) => {
    html += `<div class="card m-3" style="width: 18rem;">
        ...
        <div class="card-body">
          ...
          <button type="button" class="btn btn-danger" data-id="${user.id}">x</button>
        </div>
      </div>`
  })
...
function removeFavourite(id) {
  ...
  if (searchInput.value.length !== 0) {     //如果有搜索關鍵字，則刪除用戶后，以渲染filteredUserLists爲主；反之，userLists爲主
    let filteredUserIndex = filteredUserLists.findIndex(filteredUserList => filteredUserList.id === id)
    let userIndex = userLists.findIndex(userList => userList.id === id)
    filteredUserLists.splice(filteredUserIndex, 1)
    userLists.splice(userIndex, 1)
    renderUserLists(getUserByPage(1))     //刪除後跳回第一頁；本來想保留在原本的頁數，但想不到方法
    paginationNum(filteredUserLists)
    localStorage.setItem('favouriteUsers', JSON.stringify(userLists))
  } else {
    let userIndex = userLists.findIndex(userList => userList.id === id)
    userLists.splice(userIndex, 1)
    renderUserLists(getUserByPage(1))     //刪除後跳回第一頁；本來想保留在原本的頁數，但想不到方法
    paginationNum(userLists)
    localStorage.setItem('favouriteUsers', JSON.stringify(userLists))
  }
